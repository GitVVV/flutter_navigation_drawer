...
class ContactPage extends StatelessWidget {
          ...
          slivers: [
            ...
            SliverList(
              delegate: SliverChildListDelegate(
                <_ContactCategory>[
                  _buildPhoneCategory(),
                  _buildCategory(Icons.location_on, Icons.map, <String>[_contact.location.street, _contact.location.city]),
                  _buildCategory(Icons.contact_mail, Icons.email, <String>[_contact.email]),
                  _buildCategory(Icons.today, Icons.add_alert, <String>["Birthday ${_contact.birthday}"]),
                ],
              ),
            ),
          ],
  ...
  _ContactCategory _buildPhoneCategory() {
    var phoneItems = _contact.phones
        .map(
          (phone) => new _ContactCategoryItem(
            icon: Icons.message,
  ...
  _ContactCategory _buildCategory(IconData categoryIcon, IconData categoryItemIcon, List<String> lines) {
    return new _ContactCategory(
      icon: categoryIcon,
...
class _ContactCategoryItem extends StatelessWidget {
  ...
  _ContactCategoryItem({Key key, @required this.icon, @required this.lines}) : super(key: key);
  ...
  List<Widget> _buildRow(BuildContext context) {
    ...
    return <Widget>[
      new Flexible(
        child: new Column(crossAxisAlignment: CrossAxisAlignment.start, children: firstColumn),
      ),
      new SizedBox(
        width: 72.0,
